...
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '0').split()] #(make sure bot is admin)
...
LOG_API_CHANNEL = int(environ.get('LOG_API_CHANNEL', '-1002106653809')) #If Anyone Set Your Bot In Any Group And Set Shortner In That Group Then In This Channel The All Details Come
...
UPDATE_CHNL_LNK = environ.get('UPDATE_CHNL_LNK', '-1002756381177') # Update channel link for the bot

# ============================
# User Configuration
# ============================
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '-1002106653809').split()]
...
INDEX_REQ_CHANNEL = int(environ.get('-1002230211690', LOG_CHANNEL))  # Index Request Channel ID (make sure bot is admin)
